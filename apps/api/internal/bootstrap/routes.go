...
import (
	...
	pdfgenerate "github.com/retainr/api/internal/features/pdf/generate"
...
func Routes(cfg config.Config, pool *pgxpool.Pool, log *slog.Logger) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		// PDF API
		r.Post("/v1/pdf/generate", pdfgenerate.Handler(pool, log))
		r.Get("/v1/pdf/jobs/{jobID}", pdfgenerate.StatusHandler(pool, log))
	})

...
CREATE INDEX ON pdf_jobs (workspace_id);
CREATE INDEX ON pdf_jobs (status, created_at) WHERE status IN ('pending', 'processing');
...
CREATE POLICY pdf_jobs_workspace_isolation ON pdf_jobs
    USING (workspace_id = current_setting('app.workspace_id', true)::uuid);
...
CREATE TRIGGER pdf_jobs_updated_at BEFORE UPDATE ON pdf_jobs
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
